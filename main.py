...
def _keyword_count(text: str, keywords: list[str]) -> int:
    ...
    total = 0
    for k in keywords:
        total += lower.count(k.lower())
    return total


def _behavior_intensity(score: int) -> str:
    if score >= 8:
        return "alta"
    if score >= 4:
        return "media"
...
@app.get("/behavior/patterns")
def behavior_patterns(authorization: Optional[str] = Header(default=None)):
    """
    Análisis histórico Premium basado en chats y diarios guardados.
    Devuelve un JSON corto para que la app muestre una experiencia limpia,
    calmada y útil, sin saturar a una persona ansiosa.
    """
    decoded = verify_firebase_token(authorization)
    firebase_uid = decoded["uid"]
    user_id = f"firebase::{firebase_uid}"
    ...
    db = SessionLocal()
    try:
        entitlement = get_current_entitlement(db, firebase_uid)
        if not entitlement or not entitlement.is_active or entitlement.plan != "premium":
            raise HTTPException(
                status_code=402,
                detail="Los patrones históricos avanzados están disponibles en Premium.",
            )

        chat_rows = (
            db.query(ChatTurn)
            .filter(ChatTurn.user_id == user_id, ChatTurn.role == "user")
        ...
        texts = [r.content for r in chat_rows if r.content] + [r.text for r in journal_rows if r.text]
        corpus = " ".join(texts).lower()
        records_analyzed = len(texts)

        pattern_defs = [
            {
                "name": "Ansiedad anticipatoria",
                "keywords": ["ansiedad", "ansiosa", "ansioso", "nervios", "miedo", "preocup", "pánico", "panico"],
                "explanation": "Aparecen señales de preocupación previa o sensación de alerta antes de que algo ocurra.",
                "micro_action": "Antes de resolver el problema, haz un check-in de 60 segundos: ¿qué es real y qué está anticipando tu mente?",
            },
            {
                "name": "Sobrecarga emocional",
                "keywords": ["agotada", "agotado", "cansada", "cansado", "saturada", "saturado", "no puedo más", "demasiado", "burnout"],
                "explanation": "Se repiten palabras asociadas a cansancio, saturación o sensación de estar sosteniendo demasiado.",
                "micro_action": "Reduce la exigencia de hoy a una sola prioridad pequeña y deja el resto para después.",
            },
            {
                "name": "Rumiación mental",
                "keywords": ["pienso mucho", "no paro de pensar", "darle vueltas", "cabeza", "mente", "preocupa", "pensamientos"],
                "explanation": "Tus registros sugieren vueltas mentales repetidas o dificultad para desconectar.",
                "micro_action": "Escribe durante 3 minutos lo que se repite y termina con: “ahora mismo solo puedo hacer…”",
            },
            {
                "name": "Descanso sensible",
                "keywords": ["dormir", "insomnio", "sueño", "sueno", "desvelada", "desvelado", "noche", "cansancio"],
                "explanation": "El sueño o la noche aparecen como momentos importantes en tu estado emocional.",
                "micro_action": "Esta noche prueba una transición suave: luz baja, móvil lejos y una nota corta antes de dormir.",
            },
            {
                "name": "Sensibilidad relacional",
                "keywords": ["pareja", "relación", "relacion", "amor", "me dejó", "me dejo", "ignora", "rechazo", "discusión"],
                "explanation": "Las relaciones aparecen como un disparador emocional frecuente.",
                "micro_action": "Cuando aparezca el impulso de reaccionar, nombra primero qué dolió: silencio, rechazo, miedo o incertidumbre.",
            },
            {
                "name": "Autoexigencia",
                "keywords": ["culpa", "fracaso", "debería", "deberia", "exijo", "perfecto", "mal conmigo", "no soy suficiente"],
                "explanation": "Se observan señales de presión interna, culpa o sensación de no estar haciendo suficiente.",
                "micro_action": "Cambia “debería poder con todo” por “hoy puedo hacer una parte con calma”.",
            },
        ]

        scored = []
        for item in pattern_defs:
            score = _keyword_count(corpus, item["keywords"])
            if score > 0:
                scored.append({
                    "name": item["name"],
                    "score": score,
                    "intensity": _behavior_intensity(score),
        ...
        if not scored:
            return {
                ...
                "dominant_pattern": "Aún estamos construyendo tu mapa emocional",
                "summary": "Necesitamos más chats o entradas de diario para detectar patrones históricos con confianza.",
                "patterns": [],
                "micro_actions": [
                    "Registra cómo te sientes durante varios días.",
                    "Escribe aunque sea una frase en el diario cuando notes una emoción fuerte.",
                ],
            }

        dominant = scored[0]

        low_intensity_count = sum(1 for j in journal_rows if (j.intensity or 0) >= 7)
        extra_note = ""
        if low_intensity_count >= 3:
            extra_note = " También aparecen varios registros de intensidad alta; conviene priorizar descanso y apoyo."

        return {
            "ok": True,
            "records_analyzed": records_analyzed,
            "dominant_pattern": dominant["name"],
            "summary": f"El patrón que más se repite es {dominant['name'].lower()}. Tómalo como una señal de autoconocimiento, no como una etiqueta.{extra_note}",
            "patterns": scored[:4],
            "micro_actions": [p["micro_action"] for p in scored[:3]],
        }
    finally:
        db.close()
